...
async function handleDeploy(payload: DeployPayload) {
  ...
  // `docker exec` calls (runtime chat) do not inherit shell exports from entrypoint.sh.
  // Keep provider-native API key env vars on the container itself so every process can auth.
  if (provider === "openrouter") {
    dockerArgs.push("-e", `OPENROUTER_API_KEY=${payload.modelApiKey}`);
  } else if (provider === "openai") {
    dockerArgs.push("-e", `OPENAI_API_KEY=${payload.modelApiKey}`);
  } else if (provider === "minimax") {
    dockerArgs.push("-e", `MINIMAX_API_KEY=${payload.modelApiKey}`);
  } else if (provider === "moonshot") {
    dockerArgs.push("-e", `MOONSHOT_API_KEY=${payload.modelApiKey}`);
  } else if (provider === "nvidia") {
    dockerArgs.push("-e", `NVIDIA_API_KEY=${payload.modelApiKey}`);
  }

  let tempDir = "";
  if (payload.brandConfig) {
    try {
      const safeSkillFileName = sanitizeFileName(payload.brandConfig.skillFileName) || "SKILL.md";
      ...
      const canonicalSkillPath = join(tempDir, "SKILL.md");
      writeFileSync(canonicalSkillPath, payload.brandConfig.skillContent);

      const agentsPath = join(tempDir, "AGENTS.md");
      const agentsContent = `# Mission

You are the deployed brand operator for deployment \`${payload.deploymentId}\`.

Core rules:
- Treat \`/root/.openclaw/workspace/SKILL.md\` as the source of truth for brand voice, strategy, and API contracts.
- Read \`/root/.openclaw/workspace/SKILL.md\` before acting when context is unclear.
- Follow \`/root/.openclaw/workspace/HEARTBEAT.md\` for recurring tasks.
- Do not ask users for platform internal credentials that are already injected via environment variables.
- Stay focused on X growth/reply-guy outcomes with safe, non-spammy behavior.
`;
      ...
      dockerArgs.push("-v", `${canonicalSkillPath}:/root/.openclaw/workspace/SKILL.md:ro`);
      dockerArgs.push("-v", `${agentsPath}:/root/.openclaw/workspace/AGENTS.md:ro`);
...
async function handleChat(payload: ChatPayload) {
  ...
  const execResult = await run("docker", [
    ...
    "-e",
    `OPENCLAW_EXEC_MESSAGE=${payload.message.trim()}`,
    "-e",
    `OPENCLAW_EXEC_SESSION_ID=${payload.sessionId.trim()}`,
    "-e",
    `OPENCLAW_EXEC_CHANNEL=${payload.channel}`,
    containerName,
    "sh",
    "-lc",
    `case "\${OPENCLAW_PROVIDER:-openrouter}" in
      openrouter) export OPENROUTER_API_KEY="\${OPENCLAW_API_KEY:-}";;
      openai) export OPENAI_API_KEY="\${OPENCLAW_API_KEY:-}";;
      minimax) export MINIMAX_API_KEY="\${OPENCLAW_API_KEY:-}";;
      moonshot) export MOONSHOT_API_KEY="\${OPENCLAW_API_KEY:-}";;
      nvidia) export NVIDIA_API_KEY="\${OPENCLAW_API_KEY:-}";;
    esac
    exec openclaw agent --message "\${OPENCLAW_EXEC_MESSAGE}" --session-id "\${OPENCLAW_EXEC_SESSION_ID}" --channel "\${OPENCLAW_EXEC_CHANNEL}" --json`
  ]);

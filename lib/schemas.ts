...
export const checkoutSchema = z.object({
  ...
  model_provider: z.enum(["openrouter", "openai", "minimax", "moonshot", "nvidia"]).default("openrouter"),
    ...
    ctx.addIssue({
      ...
      message:
        input.model_provider === "openai"
          ...
          : input.model_provider === "minimax"
            ? "Starter requires your MiniMax API key."
  ...
  const isMiniMaxModel = input.default_model.startsWith("minimax/");
  ...
  if (input.model_provider === "minimax" && !isMiniMaxModel) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["default_model"],
      message: "Selected model does not match provider (expected a MiniMax model)."
    });
  }
